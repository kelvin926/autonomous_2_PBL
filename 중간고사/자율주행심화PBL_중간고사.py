...
def main():
    ...
    def make_space():
        car.turn_right()
        car.forward()
        time.sleep(1.1) # 시험시간 때 한 번 확인 1.2 -> 0.9
        car.stop()
        time.sleep(0.3)
    ...
    def out_ready():
        car.turn_right()
        car.backward()
        time.sleep(0.3) # 요것도 다시 확인 0.2 -> 0.4
        car.stop()

        car.turn_left()
        car.forward()
        time.sleep(0.3) # 요것도 다시 확인 0.2-> 0.3
        car.stop()
        time.sleep(3.0)
    ...
    def parking_out():
        ...
        car.stop()
        time.sleep(0.3)
    # processing_ultrasonic() # 초음파
###################### 구동 부분 시작 (여기 아래만 변경) ######################

#주차칸지점에서 25cm, 44.5cm 시작
    first_straight()
    parking_in()
    make_space()
    out_ready()
